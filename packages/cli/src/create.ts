...
type Template = "chat" | "copilot" | "widget" | "minimal"
type Provider = "openai" | "anthropic" | "google" | "none"

export async function create(
  projectName: string | undefined,
  options: { template?: string; provider?: string }
) {
  ...
  // Write package.json
  writeFileSync(join(projectDir, "package.json"), makePackageJson(name, provider))
  ...
  // Write tsconfig.json
  writeFileSync(join(projectDir, "tsconfig.json"), makeTsConfig())
  ...
  // Write .env.local
  writeFileSync(join(projectDir, ".env.local"), makeEnvLocal(provider))
  console.log(chalk.green("  +"), ".env.local")

  // Write API route
  writeFileSync(
    join(projectDir, "src/app/api/chat/route.ts"),
    makeApiRoute(provider)
  )
  console.log(chalk.green("  +"), "src/app/api/chat/route.ts")

  // For chat template, write chat panel component
  if (template === "chat") {
...
function makePackageJson(name: string, provider: Provider): string {
  ...
  if (provider === "openai") deps["@ai-sdk/openai"] = "^1.3.0"
  else if (provider === "anthropic") deps["@ai-sdk/anthropic"] = "^1.3.0"
  else if (provider === "google") deps["@ai-sdk/google"] = "^1.3.0"
...
function makeLayout(projectName: string): string {
...
function makePage(template: Template): string {
  if (template === "minimal") {
    return `"use client"
  ...
  return (
    <main className="min-h-screen flex">
      ...
      <aside className="w-96 border-l flex flex-col h-screen">
        <div className="px-4 py-3 border-b font-medium text-sm">Copilot</div>
        <Fabrik chat={chat}>
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen p-8">
      ...
      <p className="text-gray-600">
        Click the button in the bottom-right corner to open the chat widget.
      </p>
...
function makeEnvLocal(provider: Provider): string {
  if (provider === "openai") return "# Add your OpenAI API key here\nOPENAI_API_KEY=sk-...\n"
  if (provider === "anthropic") return "# Add your Anthropic API key here\nANTHROPIC_API_KEY=sk-ant-...\n"
  if (provider === "google") return "# Add your Google AI API key here\nGOOGLE_GENERATIVE_AI_API_KEY=...\n"
  return "# Add your LLM provider API key here\n# OPENAI_API_KEY=sk-...\n# ANTHROPIC_API_KEY=sk-ant-...\n# GOOGLE_GENERATIVE_AI_API_KEY=...\n"
}

function makeApiRoute(provider: Provider): string {
  if (provider === "openai") {
